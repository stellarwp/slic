...
4. `.env.slic.local` in your _target's_ directory - this file doesn't exist by default. Make overrides to a single project (e.g. `SLIC_PHP_VERSION`).
...
- `xoff` - Disable Xdebug
- `pcovon` - Enable PCOV
- `pcovoff` - Disable PCOV

### PCOV and `slic`

PCOV can be toggled in both the `wordpress` and `slic` containers for faster coverage collection:

- `slic pcov on` - Enable PCOV in both containers
- `slic pcov off` - Disable PCOV in both containers
- `slic pcov status` - Show the current PCOV status

The command sets the desired PCOV state and applies it to running containers immediately. Stopped containers pick up the configured state the next time they start. PCOV is disabled by default in the stack images and is intended for coverage runs, not day-to-day development.

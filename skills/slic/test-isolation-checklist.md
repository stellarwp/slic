...
3. **Factory-created data is used for test objects** — `static::factory()->post->create()` etc. lives inside the transaction and is automatically cleaned up. Avoid `wp_insert_post()` directly unless testing the insert function itself.
...
8. **No reliance on test execution order** — each test must pass when run alone (`slic run tests/wpunit/YourTest.php:test_one_method`) and when run in any order within the suite.
...
| 3 | Use factories | Manually inserted data persists if transaction fails to roll back | Replace `wp_insert_post()` with `static::factory()->post->create()` |
...
| 9 | No hardcoded IDs | Tests fail on fresh databases or after other tests create objects | Use factory return values: `$id = static::factory()->post->create()` |
